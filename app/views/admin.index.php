...
<div id="app-container">
	<section class="panel app">
		<div id="header">
			<header class="panel-heading">
				<?php if ($this->status !== false): ?>
					<div class="update-panel <?php echo ($this->status['status'] ? 'success' : 'error'); ?>">
						<h4><?php echo $this->status['message']; ?></h4>
						<button class="close-button" type="button">
							<span>×</span>
						</button>
					</div>
				<?php endif; ?>
				<h2 class="panel-title"><?php echo $this->plugin->name; ?></h2>
		...
		<div class="panel-body">
			<div id="dashboard-doc">
				<h2><?php echo __('Dashboard', 'hm'); ?></h2>
				<p>Welkom in het dashboard van de <?php echo $this->plugin->name; ?> plugin.</p>
				...
				<p>Je gebruikt momenteel versie <?php echo $this->plugin->version; ?> van de plugin.</p>
				...
				<p>Alle gegevens m.b.t. de API van Couverts kun je opvragen bij Couverts.</p>

				<form class="form" id="couvertsSettingsForm" method="POST" action="<?php echo admin_url('admin.php?page=couverts-settings'); ?>">
					<div class="row">
						...
						<div class="column column-75">
							<p>
								<input type="text" name="couvertsBaseUrl" value="<?php echo $this->plugin->settings->get('couvertsBaseUrl'); ?>">
							</p>
						</div>
						...
						<div class="column column-75">
							<p>
								<input type="text" name="couvertsRestaurantId" value="<?php echo $this->plugin->settings->get('couvertsRestaurantId'); ?>">
							</p>
						</div>
						...
						<div class="column column-75">
							<p>
								<input type="text" name="couvertsApiKey" value="<?php echo $this->plugin->settings->get('couvertsApiKey'); ?>">
							</p>
						</div>

						<div class="column column-75">
							<p>
								<input type="submit" class="btn btn-primary" value="Instellingen opslaan" name="couvertsSettingsSave">
							</p>
						</div>
					</div>
				</form>
			...
			<div id="settings-doc" style="display:none;">
				<div class="row">
					...
					<div class="pull-left col-left column column-25">
						<p>
							Huidige template map:
						</p>
					</div>
					<div class="column column-75">
						<p>
							<?php echo $this->plugin->getPartialPath(); ?>
							<?php $templatesExist = file_exists(get_template_directory().'/couverts-form-partials'); ?>
					...
					<div class="column column-75">
						<p>De Couverts plugin beschikt over een aantal templates die je zelf kunt vormgeven. Deze templates worden, normaal gesproken, geplaatst in je thema in de map 'couverts-form-partials'.</p>
						<?php if (!$templatesExist): ?>
							<p>In sommige gevallen kan het voorkomen dat de plugin deze bestanden niet kan kopi&euml;ren wanneer de toegangsrechten niet goed staan. In dat geval is het aan te raden het mapje 'couverts-form-partials' vanuit de plugin te kopi&euml;ren naar het actieve thema. Zo kun je gemakkelijk de vormgeving aanpassen, zonder dat deze verloren gaat bij een update.</p>
							<p>De thema bestanden in deze plugin zijn bereikbaar via <br/><code>/wp-content/plugins/reserveren-via-couverts/app/views/couverts-form-partials</code></p>
							<p>Kopieer vervolgens de map en de inhoud naar het actieve thema.</p>
						<?php endif; ?>
					</div>
				</div>
			</div>
				...
				<div class="row">
					<div class="pull-left col-left column column-25">
						<p>
							...
							<span class="tip-data"><?php echo __('Tenminste PHP 5.3 is noodzakelijk voor deze plugin.', 'hm'); ?></span>
						</p>
					</div>
					<div class="column column-75">
						<p>
							<?php if (!function_exists('phpversion') || phpversion() < 5.3): ?>
								<span class="dashicons dashicons-warning"></span> 
							<?php else: ?>
								<span class="dashicons dashicons-yes"></span> 
							<?php endif; ?>
					...
					<div class="column column-75">
						<p><?php echo $this->plugin->version; ?></p>
					</div>
					...
					<div class="column column-75">
						<p><?php echo $this->plugin->url; ?></p>
					</div>

					<div class="pull-left col-left column column-25">
						<p>
							<span class="has-tip dashicons dashicons-editor-help"></span> Huidige template pad:
							<span class="tip-data">
								Normaal gesproken hoort de plugin de templates te gebruiken binnen je thema. Op deze manier kun je het reservering formulier zelf vormgeven, zonder dat deze verloren gaat bij een update.
							</span>
						</p>
					</div>
					<div class="column column-75">
						<p><?php echo $this->plugin->getPartialPath(); ?></p>
					</div>
				</div>
				<div class="row">
	...
	<!-- <section class="panel feed">
		<div id="header">
			<header class="panel-heading">
				<h2 class="panel-title"><?= __('Recent news on Tussendoor.nl', 'hm'); ?></h2>
			</header>
	...
	<section class="panel feed">
		<div id="header">
			<header class="panel-heading">
				<h2 class="panel-title"><?= __('Like us on Facebook', 'hm') ?></h2>
				<p><?= __('Like us on Facebook and stay tuned on all our updates.', 'hm') ?></p>
			</header>
